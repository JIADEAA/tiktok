...
//		redis -> pipeline
    ...
    /**
     * 实现命令：ZADD key score1 member1。
     *
     * @param key
     * @param value
     * @return 向指定有序集合添加一个元素。
     */

    public boolean zAdd(String key, String value) {
        return redisTemplate.opsForZSet().add(key, value, Instant.now().toEpochMilli());
    }
     ...
     *
     * @param key
     * @return 获取指定有序集合 start 和 end 之间的元素（score 从高到底）。
     */
     ...
     * @param key
     * @param member
     * @return 获取指定有序集合中指定元素的排名(score 从大到小排序)。
     */
